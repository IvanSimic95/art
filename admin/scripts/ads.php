<?php
        ...
        $sql = "SELECT * FROM orders WHERE (order_status = 'shipped' AND fbCampaign = '$campaign' AND fbAdset = '$adset') OR (order_status = 'processing' AND fbCampaign = '$campaign' AND fbAdset = '$adset') GROUP BY fbAd ORDER BY order_id DESC";
        $result = $conn->query($sql);
                if ($result->num_rows == 0) {
                        ...
                        if($id == "domain_click" OR $id == "{{ad.id}}" OR $id == "{{ad.id" OR $id == "" OR $id == "0"){
                                        ...
                                        $sql4 = "SELECT * FROM orders WHERE (order_status = 'shipped' AND fbCampaign = '$campaign' AND fbAdset = '$adset' AND fbAd = '$id' AND DATE(order_date) >= '$startDate' AND DATE(order_date) <= '$endDate') OR (order_status = 'processing' AND fbCampaign = '$campaign' AND fbAdset = '$adset' AND fbAd = '$id' AND DATE(order_date) >= '$startDate' AND DATE(order_date) <= '$endDate')";
                                        $r4 = $conn->query($sql4);
